...
def create_example_tissue():
    ...
    tissue_dict["muscle"] = sp.define_horizontal_layer_structure_settings(z_start_mm=SPACING, thickness_mm=100,
                                                                          ...
                                                                          tissue_library.muscle(background_oxy=sp.BlobHeterogeneity(dim_x,
                                                                                                                  dim_y,
                                                                                                                  dim_z,
                                                                                                                  SPACING,
                                                                                                                  _min=0.00001,
                                                                                                                  _max=1).get_map(),
                                                                              blood_volume_fraction=sp.BlobHeterogeneity(
                                                                                  dim_x, dim_y, dim_z, SPACING,
                                                                                  _min=0.01, _max=1).get_map()),
                                                                          priority=1,
        ...
        for horizontal_idx in range(NUM_HORIZONTAL_COMPARTMENTS):
            ...
            if vessel_randomisation < vessel_probability:
                ...
                tissue_dict[f"vessel_{idx}"] = sp.define_circular_tubular_structure_settings(
                    ...
                    molecular_composition=tissue_library.blood(oxygenation=sp.RandomHeterogeneity(dim_x, dim_y, dim_z, SPACING,
