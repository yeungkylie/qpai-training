...
ground_truth1, prediction1 = get_gt_and_prediction(test_data, "Skin", "thresholded")
ground_truth2, prediction2 = get_gt_and_prediction(test_data, "Heterogeneous 0-100", "smoothed")
ground_truth3, prediction3 = get_gt_and_prediction(test_data, "Heterogeneous with vessels", "smoothed")

fig, ax = plt.subplots()
ax.plot(ground_truth1, ground_truth1, linestyle='dashed', label="True value")
ax.plot(ground_truth1, prediction1, color='g', label="Skin (Thresholded)")
ax.plot(ground_truth2, prediction2, color='darkred', label="Heterogeneous 0-100 (Smoothed)")
ax.plot(ground_truth3, prediction3, color='r', label="Heterogeneous with vessels (Smoothed)")
plt.xlabel("Ground truth oxygenation (%)")
plt.ylabel("Predicted oxygenation (%)")
ax.legend()
plt.title("Simulation 6: Forearm (Reconstructed Data)")
