...
def get_gt_and_prediction_discrete(test_data, SET_NAME, process):
    ...
    oxy_vals = np.linspace(0,1,11)
    print(oxy_vals)
    pred_oxy=np.empty((11,2094400))
    print(pred_oxy)
...
ax.plot(oxy_vals * 10 + 1, oxy_vals, 'black', label="True value")
ax.set_xticks(np.linspace(1,11,6))
...
#plot the true value of oxygenation
ax2.plot(oxy_vals * 10 + 1, oxy_vals, 'black', label="True value")
ax2.set_xticks(np.linspace(1,11,6))
